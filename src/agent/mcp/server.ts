...
function decorateMcpToolDescription(
  ...
  const writeGuidance =
    toolName === "zotero_script"
      ? "Write-mode zotero_script pauses in Zotero and shows the user the script source for approval before it runs. Write scripts must call env.snapshot(item) before mutating items, or env.addUndoStep(fn) for custom changes, so undo_last_action can revert the operation."
      : mutability === "write"
