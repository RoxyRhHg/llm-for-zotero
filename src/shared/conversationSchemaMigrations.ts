...
export async function rekeyConversationOwnedRowsInTransaction(
  ...
  try {
    // IDs generated by the conversation registry include a `legacy-<key>`
    // suffix. Keep provenance rows deletable by the new catalog identity when
    // a legacy key is remapped; arbitrary caller-supplied IDs are untouched.
    await db.queryAsync(
      `UPDATE llm_for_zotero_conversation_fork_links
       SET source_conversation_id = REPLACE(source_conversation_id, ?, ?),
           target_conversation_id = REPLACE(target_conversation_id, ?, ?)
